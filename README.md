...
| Sample-Wpf, Sample-Ava                                 | Small apps that show Words being put in the XAML and AXAML respectively. Sample-Ava is the full tour: formatting, links, image schemes, format parameters, a live markdown playground, a dark/light theme switch (watch the `dynres:` image follow it), and a language dropdown that relaunches the app in the selected language. |
